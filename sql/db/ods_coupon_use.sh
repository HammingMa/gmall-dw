...
update_time=using_time
update_time2=used_time


case $table_type in
    1)
        sql="select $cloumns from $table_name where 1=1 and "
    ;;
    2)
    ...
    3)
        ...
        if [ -n $1 ]; then
            sql="$sql or date_format($update_time2,'%Y-%m-%d')='$do_date' "
        fi
    ;;
esac
